...
def output_HITS(iteration, graph, result_dir, fname):
    ...
    print('Authority:')
    print(auth_list)
    np.savetxt(os.path.join(result_dir, fname + authority_fname), auth_list, fmt='%.3f', newline=" ")
    print('Hub:')
    print(hub_list)
    np.savetxt(os.path.join(result_dir, fname + hub_fname), hub_list, fmt='%.3f', newline=" ")
...
def output_PageRank(iteration, graph, damping_factor, result_dir, fname):
    ...
    print('PageRank:')
    print(pagerank_list)
    np.savetxt(os.path.join(result_dir, fname + pagerank_fname), pagerank_list, fmt='%.3f', newline=" ")
...
def output_SimRank(iteration, graph, decay_factor, result_dir, fname):
    ...
    print('SimRank:')
    print(ans)
    np.savetxt(os.path.join(result_dir, fname + simrank_fname), ans, delimiter=' ', fmt='%.3f')
